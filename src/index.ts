...
const WEBHOOK_SECRET_FOR_TESTING = 'prueba_key1';
...
app.post('/webhook/transaction', async (c) => {
  ...
  try {
      ...
      // // 6. Persistir los datos del webhook en la base de datos
      try {
          await db.insert(webhookTransactions).values({ ...webhookData });
          console.log('Webhook stored in DB with id', webhookData.id);
      } catch (dbError) {
          console.error('Database error while storing webhook:', dbError);
          return c.json({ error: 'Failed to store webhook data in database' }, 500);
      }
      ...
      console.log(`Processing transaction ID: ${webhookData.id} with status: ${webhookData.status}`);

      // 7. Enviar una respuesta exitosa
      // Un 200 OK es la respuesta estándar que espera el emisor del webhook.
      return c.json({ message: 'Webhook received and signature verified successfully!' });

  } catch (error) {
      console.error('Error during webhook processing:', error);
      // Manejar errores de parsing JSON o de criptografía
      return c.json({ error: 'Internal server error or invalid data format.' }, 500);
  }
});
